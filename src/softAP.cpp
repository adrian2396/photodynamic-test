...
void softAPMode::vFrontendWifiCredentials(){
  ...
  <body onload='window.scrollTo(0.5)'>\n\
    <header>\n\
      <div class='container'>\n\
        <h1 style='text-align: center;'>Photodynamic Test</h1>\n\
      </div>\n\
    </header>\n\
    <div class='disclaimer'>\n\
      <section class='text'>\n\
        <div class='description'>\n\
          <p style=''>\n\
             Please, provide internet access to the device.<br>\n\
             Enter some wifi credentials.<br>\n\
             Then go to: 172.20.10.9 .<br>\n\
        </div>\n\
      </section>\n\
    </div>\n\
    <div class='container'>\n\
  ...
  if (server->args() > 0){
    for (int i = 0; i < server->args(); i++) {
        if (server->argName(i).equals("username"))
            softAPVar.username_sta = server->arg(i);
        else if (server->argName(i).equals("password"))
            softAPVar.password_sta = server->arg(i);
    }
    if (!softAPVar.username_sta.equals("username") && !softAPVar.password_sta.equals("password")){
        // Store SSID and password in EEPROM
        eeprom_write(EEPROM_SSID_STA_KEY, softAPVar.username_sta);
        eeprom_write(EEPROM_PASWD_STA_KEY, softAPVar.password_sta);
        eeprom_commit();
        softAPVar.username_sta.toCharArray(softAPVar.username_sta_array, sizeof(softAPVar.username_sta_array));
        softAPVar.password_sta.toCharArray(softAPVar.password_sta_array, sizeof(softAPVar.password_sta_array));
        DEBUG_MSG("Station SSID: %s\n", softAPVar.username_sta_array);
        DEBUG_MSG("Station passwd: %s\n", softAPVar.password_sta_array);
